...
function [rxMMSE,rxLS,rxKF] = MRCrx(y,hmmseI,hlsI,hkI, N0)
...
% Minimum mean suqared error MRC
p = conj(hmmseI);
p2 = p/(norm(hmmseI) + N0);
...
mmsePower = mean(abs(rxGridmmse).^2);
kfPower = mean(abs(rxGridk).^2);
lsPower = mean(abs(rxGridls).^2);

rxMMSE = zeros(size(rxGridmmse,1), size(rxGridmmse,2));
rxLS = rxMMSE; rxKF = rxMMSE;

for i=1:size(rxGridmmse,2)
    rxMMSE(:,i) = rxGridmmse(:,i) / sqrt(mmsePower(1,i));
    rxLS(:,i) = rxGridls(:,i) / sqrt(lsPower(1,i));
    rxKF(:,i) = rxGridk(:,i) / sqrt(kfPower(1,i));
end   
